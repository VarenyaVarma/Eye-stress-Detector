...
export default function Signup(){
  ...
  const submit = async (e)=>{
    ...
    setError('')
    try {
      const res = await axios.post((import.meta.env.VITE_API_URL||'http://localhost:5000') + '/api/auth/signup', { name, email, password })
      if (res.data.token){
  ...
  return (
    <div className="max-w-md mx-auto">
      ...
      <form onSubmit={submit} className="space-y-3">
        <input required value={name} onChange={(e)=>setName(e.target.value)} placeholder="Full name" className="w-full p-2 rounded-md bg-slate-700/40" />
        ...
        <div className="flex items-center gap-3">
          ...
          <a href="/login" className="text-sm text-slate-400">Login if you already have an account</a>
        </div>
      </form>
    </div>
  )
